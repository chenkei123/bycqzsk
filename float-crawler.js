        ...
        this.pendingItems.forEach((item, index) => {
            ...
            row.innerHTML = `
                ...
                <div class="float-crawler-item-fields">
                    <input type="text" class="float-crawler-item-title" data-index="${index}" value="${this.uiManager.escapeHtml(item.title)}" placeholder="标题">
                    ...
                    <select class="float-crawler-item-category" data-index="${index}">${categoryOptions}</select>
                </div>
            `;

            row.querySelector('input[type="checkbox"]')?.addEventListener('change', (e) => {
                this.pendingItems[index].selected = e.target.checked;
            });

            row.querySelector('.float-crawler-item-title')?.addEventListener('input', (e) => {
                this.pendingItems[index].title = e.target.value;
            });

            row.querySelector('.float-crawler-item-category')?.addEventListener('change', (e) => {
                this.pendingItems[index].categoryId = e.target.value;
            });
    ...
    async confirmImport() {
        ...
        try {
            ...
            for (const item of selectedItems) {
                ...
                if (item.categoryId) {
                    this.uiManager.addVideoToCategory(item.categoryId, saved.id);
                }
