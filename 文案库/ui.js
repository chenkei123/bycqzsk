...
class UIManager {
    ...
    renderCategories() {
        ...
        this.categories.forEach(category => {
            ...
            catEl.innerHTML = '<span class="category-name">' + this.escapeHtml(category.name) + '</span>' +
                '<span class="category-count">(' + category.videoIds.length + ')</span>' +
                '<button class="btn-delete-category" data-category-id="' + category.id + '">×</button>';
            ...
            // 添加删除分类按钮事件
            const deleteBtn = catEl.querySelector('.btn-delete-category');
            if (deleteBtn) {
